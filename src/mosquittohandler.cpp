...
{
    std::cout << "Inside the MosquittoHandler constructor!" << std::endl;
}
...
bool MosquittoHandler::init(std::string id)
{
    ...
    m_mosquittoStruct = mosquitto_new(id.c_str(), true, obj);
    if(!m_mosquittoStruct) {
...
bool MosquittoHandler::connectToBroker(const char *host, int port)
{
    ...
    int errorNum = mosquitto_connect(m_mosquittoStruct, host, port, 60);
...
bool MosquittoHandler::loopWrite()
{
    ...
    int errorNum = mosquitto_loop_write(m_mosquittoStruct, 1);
    if(errorNum != MOSQ_ERR_SUCCESS) {
        m_lastErrorString = errorByNum(errorNum);
        return false;
    }
...
bool MosquittoHandler::loopRead()
{
    ...
    int errorNum = mosquitto_loop_read(m_mosquittoStruct, 1);
    if(errorNum != MOSQ_ERR_SUCCESS) {
        m_lastErrorString = errorByNum(errorNum);
        return false;
    }
